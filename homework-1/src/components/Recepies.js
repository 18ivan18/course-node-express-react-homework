...
const Recepies = () => {
  ...
  const getAllRecipes = async () => {
    try {
      ...
      const rec = await result.json();
      if (rec.success) {
        setRecipes([...recipes, ...rec.recipes]);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const getFilteredRecipes = () => {
    let top10 = recipes.filter(
      (recipe) =>
        recipe.tags.some((t) => tags.indexOf(t) >= 0) ||
        ...
        <center>
          ...
          >
            ...
            <div>
              <button
                data-target="modal2"
                className="btn orange pulse modal-trigger"
              ...
              <div className="col s3">
                ...
                <div>
                  ...
                  <div>
                    <button
                      className="btn blue btn-flat"
                      onClick={() => addNewAuthor(formData.author)}
                    >
                      Add author
                    </button>
                  </div>
